    ...
    async with AsyncSessionLocal() as session:
        result = await session.execute(
            select(PluginDB).where(
                PluginDB.id == service_id, PluginDB.type_id.in_(["weather", "yr_weather"])
            )
        )
        ...
        if not plugin_instance.is_running():
            try:
                ...
                raise HTTPException(
                    status_code=500, detail=f"Failed to initialize weather plugin: {str(e)}"
                )
        ...
        try:
            ...
            if weather_data is None:
                raise HTTPException(
                    status_code=501, detail="Weather plugin does not support data fetching"
                )
            # Cache the result
