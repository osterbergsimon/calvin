        ...
        async with AsyncSessionLocal() as session:
            ...
            for db_plugin in db_plugins:
                ...
                if not db_plugin.enabled:
                    # If plugin was previously registered but is now disabled, unregister and cleanup it
                ...
                try:
                    ...
                    if existing:
                        try:
                            ...
                            logger.error(
                                f"Error updating existing plugin {db_plugin.id}: {e}",
                                exc_info=True
                            )
                    ...
                    try:
                        ...
                        logger.error(
                            f"Error creating plugin instance {db_plugin.id} (type: {db_plugin.type_id}): {e}",
                            exc_info=True
                        )
                    ...
                    if plugin:
                        try:
                            ...
                            # Ensure plugin is enabled (should already be from constructor, but verify)
                            ...
                            logger.error(
                                f"Error configuring plugin {db_plugin.id}: {e}",
                                exc_info=True
                            )
                        ...
                        # Plugin creation returned None - this could be normal if plugin type isn't registered
                        # Don't disable it, just log a warning
                        logger.warning(
                            f"Plugin instance creation returned None for {db_plugin.id} (type: {db_plugin.type_id}). "
                            f"Plugin type may not be registered or plugin may not be available."
                        )
                        # Don't disable - the plugin might work later when the plugin type is registered
