...
def _parse_vevent(component) -> Optional[CalendarEvent]:
    """
    ...
    try:
        ...
        if isinstance(start_dt, datetime):
            ...
            if start_dt.tzinfo is None:
                start = start_dt.replace(tzinfo=timezone.utc)
            else:
                start = start_dt
        else:
            # Date-only (all-day event) - use UTC midnight
            start = datetime.combine(start_dt, datetime.min.time(), tzinfo=timezone.utc)
        ...
        if isinstance(end_dt, datetime):
            ...
            end = datetime.combine(actual_end_date, datetime.max.time(), tzinfo=timezone.utc)
