...
describe("filterAvailableScreens", () => {
  ...
  it("null availableIds => all", () => {
    expect(filterAvailableScreens(screensConfig, null).screens.map(s => s.id)).toEqual([
      "a",
      "b",
      "c",
    ]);
  ...
  it("ids not in catalog are ignored", () => {
    expect(filterAvailableScreens(screensConfig, ["a", "zzz"]).screens.map(s => s.id)).toEqual([
      "a",
    ]);
  });
  it("empty intersection fails open to all", () => {
    expect(filterAvailableScreens(screensConfig, ["zzz"]).screens.map(s => s.id)).toEqual([
      "a",
      "b",
      "c",
    ]);
  ...
  it("empty array availableIds => all (fail open)", () => {
    expect(filterAvailableScreens(screensConfig, []).screens.map(s => s.id)).toEqual([
      "a",
      "b",
      "c",
    ]);
...
describe("resolveKioskActiveScreen", () => {
  ...
  it("empty catalog is normalized to a fallback screen — resolver returns its id, not null", () => {
    const result = resolveKioskActiveScreen({
      screensConfig: { version: 2, activeScreenId: null, screens: [] },
      availableScreens: null,
      defaultScreenId: null,
      current: null,
    });
